...
Ask Ubuntu is an AI assistant for Ubuntu Linux. It answers questions about your system, installed packages, running services, hardware, configuration, and general Ubuntu usage. It runs entirely locally — no cloud, no internet required for chat.
...
On the very first launch, Ask Ubuntu will:
...
|---------|-------------|
| `/model` | Open the interactive model picker — change the AI model |
...
Type `/model` to open a full-screen interactive picker:
...
Models are sorted with the best choice for your hardware at the top. Badges tell you:
...
When you select a model that isn't downloaded yet, Ask Ubuntu downloads it and shows progress before switching.
...
Click the **⊙** (model/sunburst) icon in the top of the left sidebar. This opens the model picker overlay:
...
Ask Ubuntu runs on Ubuntu 22.04 LTS (Jammy) and 24.04 LTS (Noble). It requires Python 3.10+ and Lemonade Server running locally on port 8000.
